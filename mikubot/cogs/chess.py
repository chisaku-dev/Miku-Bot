...
class Searcher:
    ...
    def search(self, pos, history=()):
        ...
        if DRAW_TEST:
            ...
            # await ctx.send('# Clearing table due to new history')
            self.tp_score.clear()
...
async def print_board(ctx, pos):
    ...
    for i, row in enumerate(pos.board.split()):
        line = line + '\n———————————————————————————————————————————\n' + str(8-i) + ' | ' + (' | ').join(uni_pieces.get(p, p) for p in row) + ' | '
    line = line + '\n———————————————————————————————————————————\n  |  a |  b |  c |  d |  e |  f |  g |  h |'
    ...
    while True:
        ...
        while move not in hist[-1].gen_moves():
            ...
            if match:
                ...
                await ctx.send("Please enter a move like g8f6")
        hist.append(hist[-1].move(move))

        # After our move we rotate the board and await ctx.send it again.
        # This allows us to see the effect of our move.
        await print_board(ctx, hist[-1].rotate())
